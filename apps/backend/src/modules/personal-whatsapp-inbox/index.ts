...
function normalizeMessageMediaAttributes(value: unknown) {
	...
	const mediaChanged = Boolean(
		currentUrl && normalizedUrl && normalizedUrl !== currentUrl,
	)

	const linkPreview = asRecord(attributes.link_preview)
	const currentThumbnail =
		typeof linkPreview.thumbnail_url === 'string'
	...
	const previewChanged = Boolean(
		currentThumbnail &&
			normalizedThumbnail &&
			normalizedThumbnail !== currentThumbnail,
	)
		...
		data: {
			additional_attributes: {
				...
				personal_whatsapp: {
					...
					// F4: AI handoff brief seeded by lead-routing's assign() - lets the
					// chat UI show it without a separate task lookup.
...
export const personalWhatsappInbox = new Elysia({
	prefix: '/personal-whatsapp-inbox',
})
	.use(appContext)
	.post(
		'/ingest',
		async ({ body, headers, set }) => {
			...
			if (!channelKey || !secret) {
				set.status = 403
				return { error: 'Channel authentication required' }
			}
			...
			if (!channel) {
				set.status = 403
				return { error: 'Invalid channel credentials' }
			}
			if (!channel.app_id) {
				set.status = 422
				return { error: 'Channel is not assigned to a CRM app' }
			}
			if (String(payload?.event || '') === 'presence.update') {
				...
				getRealtimeIO()
					?.to(`app:${channel.app_id}`)
					.emit('whatsapp:presence', { phone, presence, timestamp })
				if (phone) {
					const contact = await prisma.contacts.findFirst({
						where: {
							app_id: channel.app_id,
							deleted_at: null,
							OR: [{ phone_number: phone }, { whatsapp_id: phone }],
						},
						select: { id: true, additional_attributes: true },
					})
					if (contact) {
							...
							data: {
								additional_attributes: {
									...
									...(presence !== 'unavailable'
										? {
												whatsapp_last_online_at: new Date(
													timestamp,
												).toISOString(),
											}
										: {}),
			...
			if (String(payload?.event || '') === 'contact.name_changed') {
				...
				getRealtimeIO()
					?.to(`app:${channel.app_id}`)
					.emit('contact:profile_updated', { contactId: contact.id, name })
			...
			if (String(payload?.event || '') === 'call.missed') {
				...
				if (callId) {
					const alreadyLogged = await prisma.messages.findFirst({
						where: {
							...
							content_attributes: { path: ['call_id'], equals: callId },
						},
						select: { id: true },
					})
					if (alreadyLogged)
						return { success: true, updated: false, duplicate: true }
				}
				...
				getRealtimeIO()
					?.to(`app:${channel.app_id}`)
					.emit('message:created', realtimePayload)
				getRealtimeIO()
			...
			if (inboundPhone) {
				const contact = await prisma.contacts.findFirst({
					where: {
						app_id: channel.app_id,
						deleted_at: null,
						OR: [{ phone_number: inboundPhone }, { whatsapp_id: inboundPhone }],
					},
					select: { id: true },
				})
				if (contact)
					void enqueueProfileContact(
						channel.app_id,
						channel.id,
						contact.id,
						false,
	...
	.post('/repair-queue', async ({ resolvedAppId, userId, set }) => {
		...
		for (const queue of queues) {
			...
			for (const job of jobs) {
				...
				if (String(payload?.channelKey || '') !== session.provider_channel_key)
					continue
	...
	.post(
		'/leads/:registrationId/confirm',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			}
			const registration = await setPersonalLeadStatus({
				appId: resolvedAppId,
				ownerUserId: userId,
			...
			if (!registration) {
				set.status = 404
				return { error: 'Permintaan lead tidak ditemukan' }
			}
			await updateConversationPersonalLeadState(
				resolvedAppId,
				userId,
				registration,
			)
			getRealtimeIO()
				?.to(`app:${resolvedAppId}`)
				.emit('personal-lead:updated', {
			...
			if (registration.conversation_id) {
				await NotificationService.resolve(
					resolvedAppId,
					userId,
					`lead_pending:${registration.conversation_id}`,
				)
				...
				if (latestInbound) {
					void PersonalAiReplyService.scheduleInbound({
						...
						console.warn(
							'[PersonalWhatsAppInbox] Failed resuming AI after lead confirmation:',
							error,
						)
					})
					...
					void enqueueTaskAnalysis({
						...
						console.warn(
							'[PersonalWhatsAppInbox] Failed enqueueing task analysis after lead confirmation:',
							error,
						)
					})
				}
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			const { session, channel } = await resolveOwnedChannel(
				resolvedAppId,
				userId,
			)
			...
			if (session?.status === 'connected' && channel?.api_key) {
				try {
					await BaileysServiceClient.updateBlockStatus(
						{
					...
					console.warn(
						'[PersonalWhatsAppInbox] CRM block saved, but WhatsApp block failed:',
						error,
					)
				}
			}
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			if (!registration) {
				set.status = 404
				return { error: 'Nomor terblokir tidak ditemukan' }
			}
			await updateConversationPersonalLeadState(
				resolvedAppId,
				userId,
			...
			if (session?.status === 'connected' && channel?.api_key) {
				try {
					...
					console.warn(
						'[PersonalWhatsAppInbox] CRM unblock saved, but WhatsApp unblock failed:',
						error,
					)
				}
			}
	...
	.patch(
		...
		async ({ resolvedAppId, userId, body, set }) => {
			...
			const settings = await PersonalAiReplyService.updateSettings(
				resolvedAppId,
				userId,
				body,
			)
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, body, set }) => {
			...
			try {
				const message = await PersonalAiReplyService.sendDraft(
					resolvedAppId,
					userId,
					params.taskId,
					body.content,
		...
		},
		{
			...
			body: t.Object({
				content: t.Optional(t.String({ minLength: 1, maxLength: 4096 })),
			}),
		},
	)
	.post(
		'/ai/drafts/:taskId/dismiss',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			}
			const dismissed = await PersonalAiReplyService.dismissDraft(
				resolvedAppId,
				userId,
				params.taskId,
			)
	...
	.get('/unread-count', async ({ resolvedAppId, userId, set }) => {
		...
		const count = await cached(
			`count:pwa-unread:${resolvedAppId}:${userId}`,
			5,
			async () => {
				const { session, channel } = await resolveOwnedChannel(
					resolvedAppId,
				...
				const confirmedPhones = await listConfirmedPersonalLeadPhones(
					resolvedAppId,
					userId,
				)
				...
				return prisma.conversations.count({
					where: {
						...
						contacts: {
							is: {
								OR: [
									{ phone_number: { in: confirmedPhones } },
									{ whatsapp_id: { in: confirmedPhones } },
								],
							},
						},
					},
	...
	.get('/conversations', async ({ resolvedAppId, userId, set }) => {
		...
		if (!session || !channel?.inbox_id) {
			return {
				data: [],
				diagnostic: {
					connection: session?.status || 'not_paired',
					storedMessages: 0,
				},
			}
		}
		...
		const storedMessages = visibleRows.length
		...
		const takeoverByConversation = new Map<
			string,
			{ aiHandling: boolean; source: string | null }
		>()
	...
	.get('/takeovers/count', async ({ resolvedAppId, userId, set }) => {
		...
		const count = await cached(
			`count:pwa-takeover:${resolvedAppId}:${userId}`,
			5,
			async () => {
				const user = await prisma.users.findFirst({
					where: { id: userId, app_id: resolvedAppId, deleted_at: null },
	...
	.get(
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			if (!item) {
				set.status = 404
				return { error: 'Alih tugas tidak ditemukan' }
			}
			return { data: item }
		},
	)
	.post(
		...
		async ({ resolvedAppId, userId, params, body, set }) => {
			...
			if (!conversation) {
				set.status = 404
				return { error: 'Percakapan tidak ditemukan' }
			}
			const personal = ((
			...
			if (!result) {
				set.status = 404
				return { error: 'Lead tidak ditemukan untuk dialihkan' }
			}
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			if (!existing) {
				set.status = 404
				return { error: 'Lead tidak ditemukan untuk diabaikan' }
			}
			const registration = await setPersonalLeadStatus({
				appId: resolvedAppId,
				ownerUserId,
	...
	.post(
		...
		},
		{
			params: t.Object({ conversationId: t.String() }),
			body: t.Optional(
				t.Object({ note: t.Optional(t.String({ maxLength: 2000 })) }),
			),
		},
	)
	.get(
		'/:conversationId/takeover-history',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			}
			const data = await PersonalTakeoverService.history(
				resolvedAppId,
				params.conversationId,
			)
			return { data }
	...
	.get(
		'/:conversationId/lead-need',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			}
			const result = await PersonalAiReplyService.getLeadNeed(
				resolvedAppId,
				params.conversationId,
			)
			if (!result) {
	...
	.patch(
		...
		async ({ resolvedAppId, userId, params, body, set }) => {
			...
			const result = await PersonalAiReplyService.updateLeadNeed(
				resolvedAppId,
				params.conversationId,
				body,
			)
		...
		},
		{
			...
			body: t.Object({
				...
				seats: t.Optional(
					t.Nullable(t.Union([t.Number(), t.String({ maxLength: 20 })])),
				),
				budget: t.Optional(t.Nullable(t.String({ maxLength: 200 }))),
				urgency: t.Optional(t.Nullable(t.String({ maxLength: 20 }))),
				timeline: t.Optional(t.Nullable(t.String({ maxLength: 200 }))),
				painPoints: t.Optional(
					t.Array(t.String({ maxLength: 200 }), { maxItems: 5 }),
				),
				decisionRole: t.Optional(t.Nullable(t.String({ maxLength: 200 }))),
	...
	// F1: manual "Kualifikasi ulang" - queued (non-blocking), never runs inline.
	// The frontend polls getLeadNeed briefly after queuing since there's no
	// realtime listener wired up for `lead-need:updated` today.
	.post(
		'/:conversationId/lead-need/qualify',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
	...
	// F4: AI handoff brief (summary + suggested opener) seeded at assign() time,
	// read off the conversation's own additional_attributes - no task lookup.
	.get(
		'/:conversationId/handoff-brief',
		async ({ resolvedAppId, userId, params, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			...
			const brief =
				personal.handoff_brief && typeof personal.handoff_brief === 'object'
					? (personal.handoff_brief as Record<string, unknown>)
					: null
			return { data: brief }
		},
		{ params: t.Object({ conversationId: t.String() }) },
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			if (session && channel.api_key && recipient && messageIds.length) {
				void BaileysServiceClient.markMessagesRead(
					{
						channelKey: session.provider_channel_key,
					...
					console.warn(
						'[PersonalWhatsAppInbox] WhatsApp read receipt failed:',
						error,
					),
				)
			}
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, body, set }) => {
			...
			if (!session || !channel?.inbox_id || !channel.api_key) {
				set.status = 404
				return { error: 'WhatsApp belum terhubung' }
			}
			const conversation = await prisma.conversations.findFirst({
			...
			if (!recipient) {
				set.status = 404
				return { error: 'Kontak WhatsApp tidak ditemukan' }
			}
			await BaileysServiceClient.sendPresence(
				{
					channelKey: session.provider_channel_key,
					recipientWhatsAppId: recipient,
				...
				const contact = existingContact
					? await tx.contacts.update({
							...
							data: {
								...
								name: body.name?.trim() || existingContact.name || phoneNumber,
								phone_number: phoneNumber,
								whatsapp_id: phoneNumber,
								channel_type: 'whatsapp',
								deleted_at: null,
					...
					: await tx.contacts.create({
							data: {
								app_id: resolvedAppId,
								identifier,
								name: body.name?.trim() || phoneNumber,
								phone_number: phoneNumber,
								whatsapp_id: phoneNumber,
								channel_type: 'whatsapp',
	...
	.post(
		...
		async ({ resolvedAppId, userId, body, set }) => {
			...
			if (audio.size > 8 * 1024 * 1024) {
				set.status = 413
				return { error: 'Rekaman terlalu besar' }
			}
			try {
				...
				return {
					error:
						error instanceof Error ? error.message : 'Transkripsi suara gagal',
				}
			}
		},
		{
			body: t.Object({
				audio: t.File(),
				language: t.Optional(
					t.Union([t.Literal('id'), t.Literal('en'), t.Literal('auto')]),
				),
			}),
		},
	)
		...
		async ({ resolvedAppId, userId, params, set }) => {
			...
			const message = await prisma.messages.findFirst({
				where: {
					...
					app_id: resolvedAppId,
					deleted_at: { not: null },
					conversations: { is: { channel_type: 'whatsapp' } },
				},
				select: {
					id: true,
					conversation_id: true,
			...
			if (!message) {
				set.status = 404
				return { error: 'Pesan trash tidak ditemukan' }
			}
			...
			await prisma.$transaction(async (tx) => {
				...
				if (message.conversation_id) {
					...
					await tx.conversations.update({
						...
						data: {
							last_message_at: latest?.created_at || now,
							updated_at: now,
						},
					})
				}
	...
	.post(
		...
		async ({ resolvedAppId, userId, params, body, set }) => {
			...
			if (session.status !== 'connected') {
				set.status = 409
				return { error: 'WhatsApp sedang tidak terhubung' }
			}
			...
			if (
				...
				(!quotedMessage || !quotedMessage.external_id)
			) {
				set.status = 409
				...
				const sent = await BaileysServiceClient.sendMessage(
					{
						...
						...(media
							? {
									...
									isAnimated:
										media.kind === 'sticker' && media.animated === true,
								}
							: {}),
						...(quotedMessage
							? {
									quote: {
										...
										timestamp: quotedMessage.created_at
											? Math.floor(quotedMessage.created_at.getTime() / 1000)
											: undefined,
									},
								}
							: {}),
								...
								...(quotedMessage
									? {
											quote: {
												...
												external_id: quotedMessage.external_id,
												content: quotedMessage.content,
												content_type: quotedMessage.content_type,
												sender_type: quotedMessage.sender_type,
											},
										}
									: {}),
							},
					...
					if (media?.url) {
						await tx.media_files.updateMany({
							where: {
								app_id: resolvedAppId,
								message_id: null,
								OR: [{ media_url: media.url }, { local_url: media.url }],
							},
	...
	.delete(
		...
		},
		{
			params: t.Object({ conversationId: t.String() }),
			body: t.Object({
				messageIds: t.Array(t.String(), { minItems: 1, maxItems: 100 }),
			}),
		},
	)
	.delete(
	...
	.get(
		'/:conversationId/messages',
		async ({ resolvedAppId, userId, params, query, set }) => {
			if (!resolvedAppId || !userId) {
				set.status = 401
				return { error: 'Sesi CRM tidak valid' }
			}
			...
			const rows = await prisma.messages.findMany({
				where: {
					conversation_id: conversation.id,
					deleted_at: null,
					...(before && !Number.isNaN(before.valueOf())
			...
			const data = rows.reverse().map((message) => {
				const normalized = normalizeMessageMediaAttributes(
					message.content_attributes,
				)
				if (normalized.changed) {
					void prisma.messages
						.update({
							where: { id: message.id },
							data: {
								content_attributes: normalized.value as any,
								updated_at: new Date(),
							},
						})
						.catch(() => undefined)
			...
			const contactAttributes = asRecord(
				conversation.contacts?.additional_attributes,
			)
			const presence = {
				status:
					typeof contactAttributes.whatsapp_presence === 'string'
						? contactAttributes.whatsapp_presence
						: null,
				lastOnlineAt:
					typeof contactAttributes.whatsapp_last_online_at === 'string'
						? contactAttributes.whatsapp_last_online_at
						: null,
			}
