		...
		try {
			...
			setError(
				reason instanceof Error
					? reason.message
					: 'Daftar tugas belum dapat dimuat.',
			)
		} finally {
			setLoading(false)
		}
		...
		async (task: Task, action: 'start' | 'complete') => {
			...
			try {
				...
				setError(
					reason instanceof Error
						? reason.message
						: 'Status tugas belum dapat diperbarui.',
				)
			} finally {
				...
				<div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Urut berdasarkan tenggat waktu terdekat.
						</p>
					</div>
										...
										<div className="flex shrink-0 flex-wrap gap-2">
											{task.status === 'open' ? (
												<button
													type="button"
													className="ocm-btn"
													onClick={() => void updateStatus(task, 'start')}
											...
											{task.status === 'open' || task.status === 'in_progress' ? (
												<button
													...
													onClick={() => void updateStatus(task, 'complete')}
													disabled={isProcessing}
												>
